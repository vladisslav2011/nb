...
class QRVA_table extends dom_div
{
	function __construct()
	{
		parent::__construct();
		...
		$this->hcell=new dom_td;
		$this->row->append_child($this->hcell);
		$this->htext=new dom_statictext;
		$this->hcell->append_child($this->htext);
	...
	function bootstrap()
	{
		global $ddc_tables,$sql;
		$this->editors=Array();
		$this->cell->nodes=Array();
		$this->long_name=editor_generic::long_name();
		$tbl=$this->args['ed_table'];
		$db=$this->args['ed_db'];
		if($db != '')
			$xdb="`".$sql->esc($db)."`.";
		if(($db=='') && (isset($ddc_tables[$tbl])))
		{
			foreach($ddc_tables[$tbl]->keys as $key)
			{
				if($key['key']=='PRIMARY')$primary[$key['name']]=1;
			}
			foreach($ddc_tables[$tbl]->cols as $col)
			{
				if(isset($col['editor']))
				{
					$en=$col['editor'];
					...
					if($primary[$col['name']]==1)
						$ed=new editor_statictext;
					else
						$ed=new editor_text;
				}
				editor_generic::addeditor('-'.$row["Field"],$ed);
				$this->cell->append_child($this->editors['-'.$row["Field"]]);
			}
		}else{
