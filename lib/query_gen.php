...
function result()
{
	...
	if(preg_match('/^insert.*/i',$this->type))
	{
		...
		{
			if(count($this->into->exprs)!=1)return "error: insert-select into multiple columns not implemented yet";
			$res.="INTO ".$into_part;
			unset($al_list);
			foreach($this->select->what->exprs as $e)
			{
				if($al_list!='')$al_list.=',';
				$al_list.=("`".$e->alias."`");
			}
			$res.="(".$al_list.") ";
			$res.=$this->select->result();
			return $res;
